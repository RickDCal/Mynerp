Ext.define('Mynerp.view.financeiro.contaPagar.ContaPagarController', {
    extend: 'Mynerp.view.base.ViewController',

    alias: 'controller.contaspagar',

    requires: [
        'Mynerp.view.financeiro.contaPagar.ContaPagarWindow'
    ],
    ...
    onSave: function(button, e, options){
        ...
        var me = this,
            dialog = me.dialog,
            view = me.getView(),
            form = me.lookupReference('contaPagarForm'),
            grid = me.lookupReference('gridParcelasCP'),
            isEdit = me.isEdit,
            session = me.getSession(),
            associatedStore = grid.getStore(),
            //store = me.getStore('parcelaspagar'),
            store = me.getStore('contaspagar'),
            ...
            if (batch){
                ...
                batch.on("complete", function ( batch, operation, eOpts) {
                   try {
                        ...
                        me.lookupReference('gridParcelasCP').sort("sequencial", "ASC");
                        //me.getStore('contaspagar').sync();
                   } catch (e) {
    ...
    onContaSelect: function(id){
        var me = this,
            grid = me.lookupReference('parcelaPagarGrid'),
            store = me.getStore('parcelaspagar'),
    ...
    onItemClick: function( view, record, item, index, e, eOpts ) {
        this.redirectTo('contaspagar/' + record.get('id'));
    },
    ...
    //             if (entity == 'ParcelaPagar' || entity == 'ParcelaPagar') {                    
    ...
    doDialog: function (record) {
        ...
        me.dialog = view.add({
                ...
                viewModel: {
                    data: {
                         title: record ? 'Editar: ' + record.get('id') + ' - Fornecedor: ' + record.get('nomePessoa') : 'Cadastrar',
                         glyph: record ? glyphs.getGlyph('edit') : glyphs.getGlyph('add'),
    ...
    getRecordSelected: function(button) {
        var grid  = button.up('parcela-pagar-grid');
    ...
    refresh: function(button, e, options) {
        var me = this,
            store = me.getStore('parcelaspagar');
            store2 = me.getStore('contaspagar');
        store.load();
        store2.load();
    ...
    onAddParcelaClick: function(button, e, options){        
        var me = this,
            ...
            record = new Mynerp.model.financeiro.ParcelaPagar({
                sequencial: seq,
    ...
    //não cnosegui descobrid o porque então estou tirando esta parcela duplicada depois que o grid é desenhado. 
        ...
        var me = this,
        gridParcela = me.lookupReference('gridParcelasCP'),
