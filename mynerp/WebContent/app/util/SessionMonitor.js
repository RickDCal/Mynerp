 ...
 * logged out.
 */
Ext.define('Mynerp.util.SessionMonitor', {
  singleton: true,
  ...
  window: Ext.create('Ext.window.Window', {
    ...
    modal: true,
    resizable: false,
    title: 'Aviso de Inatividade da Sessão',
    width: 325,
    items: [{
      xtype: 'container',
      frame: true,
    ...
    buttons: [{
      ...
      handler: function() {
        Ext.TaskManager.stop(Mynerp.util.SessionMonitor.countDownTask);
        Mynerp.util.SessionMonitor.window.hide();
        Mynerp.util.SessionMonitor.start();
        // 'poke' the server-side to update your session.
        Ext.Ajax.request({
          url: 'userAuthenticationServlet?action=sessionRefresh' //deve ter algum jeito de consertar esta url sem ter que colocar o masteringextjs: é só não colocar a barra antes
        });
      }
    },{
      text: 'Logout',
